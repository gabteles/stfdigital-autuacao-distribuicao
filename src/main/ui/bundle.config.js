...
module.exports = {
  bundle: {
    'distribuicao': {
      scripts: [path.join(conf.paths.app, 'distribuicao.ts'),
                path.join(conf.paths.app, 'distribuicao/**/*.ts'), libraryTypeScript],
      options: {
    	  rev: false,
    	  transforms: {
              scripts: lazypipe()
              	.pipe(typescript, createTsProject())
              	.pipe(moduleNameInjector, {rootDir: 'src/main/ui/app/', prefix: 'distribuicao/'})
              	.pipe(ngAnnotate)
              	.pipe(embedTemplates, {
              		skipErrors: true, 
              		minimize: {
