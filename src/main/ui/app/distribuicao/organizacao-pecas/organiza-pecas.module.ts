...
function config($stateProvider: IStateProvider,
    ...
    /*.state('app.tarefas.organizacao-pecas.dividir-peca', {
        parent: 'app.tarefas.organizacao-pecas',
        url: '/dividir-peca',
        views: {
            '@app.tarefas.organizacao-pecas': {
                templateUrl: './dividir-peca.tpl.html',
                controller: 'app.distribuicao.organizacao-pecas.DividirPecasController'
            }
        }
    });*/
...
let pecas: IModule = angular.module('app.distribuicao.organizacao-pecas', ['app.support', 'ui.sortable', 'ngCookies', 'checklist-model']);
pecas.config(config);
pecas.run(run);
